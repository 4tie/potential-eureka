...
async def list_ai_suggestions(run_id: str) -> dict[str, Any]:
    state = _pl.get_state(run_id)
    if state is None:
        raise HTTPException(status_code=404, detail=f"Pipeline run '{run_id}' not found.")
...
async def explain_stage(run_id: str, body: ExplainStageRequest) -> dict[str, Any]:
    state = _pl.get_state(run_id)
    if state is None:
        raise HTTPException(status_code=404, detail=f"Pipeline run '{run_id}' not found.")
...
async def explain_failure(run_id: str, body: ExplainFailureRequest) -> dict[str, Any]:
    state = _pl.get_state(run_id)
    if state is None:
        raise HTTPException(status_code=404, detail=f"Pipeline run '{run_id}' not found.")
